...
```
cd ..
diff -ur --exclude=".git" a b > $GOPATH/src/gitlab.waterfall.network/waterfall/protocol/coordinator/third_party/YOURPATCH.patch
```

Next, we need to tell the Bazel [WORKSPACE](https://gitlab.waterfall.network/waterfall/protocol/coordinator/blob/master/WORKSPACE) to patch the specific dependency.
