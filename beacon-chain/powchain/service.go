...
func NewService(ctx context.Context, opts ...Option) (*Service, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "unable to retrieve Waterfall data")
	}
...
func (s *Service) Start() {
	...
	if !s.chainStartData.Chainstarted && s.cfg.currHttpEndpoint.Url == "" {
		...
		if genState == nil || genState.IsNil() {
			log.Fatal("cannot create genesis state: no Waterfall http endpoint defined")
		}
	}
...
func (s *Service) processBlockHeader(header *gwatTypes.Header) {
	...
	if header.Nr() == 0 && header.Height != 0 {
		...
		}).Warn("Latest Waterfall chain event: skipping not finalized block")
		return
	}
	...
	log.WithFields(logrus.Fields{
		...
		"blockHash":   hexutil.Encode(s.latestEth1Data.BlockHash),
	}).Info("Latest Waterfall chain event")
}
...
func safelyHandlePanic() {
	if r := recover(); r != nil {
		log.WithFields(logrus.Fields{
			"r": r,
		}).Error("Panicked when handling data from Waterfall Chain! Recovering...")
		...
		default:
			...
			if header.Nr() == 0 && header.Height != 0 {
				log.WithFields(logrus.Fields{
					...
					"header.Height": header.Height,
					"blockHash":     hexutil.Encode(s.latestEth1Data.BlockHash),
				}).Fatal("Latest Waterfall block is not finalized")
			}
...
func (s *Service) run(done <-chan struct{}) {
	...
	for {
		...
		case <-s.headTicker.C:
			...
			if err != nil {
				s.pollConnectionStatus(s.ctx)
				log.WithError(err).Error("Could not fetch latest Waterfall header")
				continue
			}
...
func (s *Service) ensureValidPowchainData(ctx context.Context) error {
	...
	eth1Data, err := s.cfg.beaconDB.PowchainData(ctx)
	if err != nil {
		return errors.Wrap(err, "unable to retrieve Waterfall data")
	}
