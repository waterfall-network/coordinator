...
func TestStore_OnAttestation_Ok_ProtoArray(t *testing.T) {
	...
	require.NoError(t, service.cfg.ForkChoiceStore.InsertOptimisticBlock(ctx, 0, tRoot, tRoot, 1, 1, params.BeaconConfig().ZeroHash[:], params.BeaconConfig().ZeroHash[:], nil, nil, nil))
...
func TestVerifyFinalizedConsistency_IsCanonical(t *testing.T) {
	...
	require.NoError(t, service.cfg.ForkChoiceStore.InsertOptimisticBlock(ctx, b32.Block.Slot, r32, [32]byte{}, 0, 0, params.BeaconConfig().ZeroHash[:], params.BeaconConfig().ZeroHash[:], nil, nil, nil))
	require.NoError(t, service.cfg.ForkChoiceStore.InsertOptimisticBlock(ctx, b33.Block.Slot, r33, r32, 0, 0, params.BeaconConfig().ZeroHash[:], params.BeaconConfig().ZeroHash[:], nil, nil, nil))
