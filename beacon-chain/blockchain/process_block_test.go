...
func TestCachedPreState_CanGetFromStateSummary_ProtoArray(t *testing.T) {
	...
	s, err := v1.InitializeFromProto(&ethpb.BeaconState{Slot: 1, GenesisValidatorsRoot: params.BeaconConfig().ZeroHash[:], BlockRoots: [][]byte{b.Block.StateRoot}})
	require.NoError(t, err)
...
func TestCachedPreState_CanGetFromDB(t *testing.T) {
	...
	b.Block.ParentRoot = gRoot[:]
	s, err := v1.InitializeFromProto(&ethpb.BeaconState{Slot: 1, BlockRoots: [][]byte{b.Block.StateRoot}})
...
func TestAncestor_CanUseForkchoice(t *testing.T) {
	...
	for _, b := range []*ethpb.SignedBeaconBlock{b1, b100, b200} {
		...
		require.NoError(t, err)
		require.NoError(t, service.cfg.ForkChoiceStore.InsertOptimisticBlock(context.Background(), b.Block.Slot, r, bytesutil.ToBytes32(b.Block.ParentRoot), 0, 0, params.BeaconConfig().ZeroHash[:], params.BeaconConfig().ZeroHash[:], nil)) // Saves blocks to fork choice store.
	}
...
func TestAncestor_CanUseDB(t *testing.T) {
	...
	require.NoError(t, service.cfg.ForkChoiceStore.InsertOptimisticBlock(context.Background(), 200, r200, r200, 0, 0, params.BeaconConfig().ZeroHash[:], params.BeaconConfig().ZeroHash[:], nil))
...
func TestOnBlock_CanFinalize(t *testing.T) {
	t.Skip() //AppendPreviousEpochAttestations is not supported for hard fork 1 beacon state
	ctx := context.Background()
	beaconDB := testDB.SetupDB(t)
	fcs := protoarray.New(0, 0)
...
func TestInsertFinalizedDeposits(t *testing.T) {
	...
	for i := uint64(0); i < uint64(4*params.BeaconConfig().SlotsPerEpoch); i++ {
		...
		assert.NoError(t, depositCache.InsertDeposit(ctx, &ethpb.Deposit{Data: &ethpb.Deposit_Data{
			...
			Amount:                0,
			Signature:             zeroSig[:],
			InitTxHash:            params.BeaconConfig().ZeroHash[:],
	...
	assert.Equal(t, -1, int(fDeposits.MerkleTrieIndex), "Finalized deposits not inserted correctly")
