...
func TestAncestor_CanUseForkchoice(t *testing.T) {
	...
	for _, b := range []*ethpb.SignedBeaconBlock{b1, b100, b200} {
		...
		require.NoError(t, err)
		require.NoError(t, service.cfg.ForkChoiceStore.InsertOptimisticBlock(context.Background(), b.Block.Slot, r, bytesutil.ToBytes32(b.Block.ParentRoot), 0, 0, params.BeaconConfig().ZeroHash[:], params.BeaconConfig().ZeroHash[:], nil, nil, nil)) // Saves blocks to fork choice store.
	}
...
func TestAncestor_CanUseDB(t *testing.T) {
	...
	require.NoError(t, service.cfg.ForkChoiceStore.InsertOptimisticBlock(context.Background(), 200, r200, r200, 0, 0, params.BeaconConfig().ZeroHash[:], params.BeaconConfig().ZeroHash[:], nil, nil, nil))
