...
func (s *Service) Status() error {
	...
	if headEpoch := slots.ToEpoch(s.cfg.chain.HeadSlot()); headEpoch+1 < slots.ToEpoch(s.cfg.chain.CurrentSlot()) && //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	...
	for {
		...
		case evt := <-stateChannel:
			...
			case statefeed.Synced:
				...
				currentEpoch := slots.ToEpoch(slots.CurrentSlot(uint64(s.cfg.chain.GenesisTime().Unix()))) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
