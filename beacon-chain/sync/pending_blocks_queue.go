...
func (s *Service) processPendingBlocks(ctx context.Context) error {
	...
	for _, slot := range pendingSlots {
		...
		if slot > s.cfg.chain.CurrentSlot() { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
			continue
		}
		...
		for _, b := range bs {
			...
			if err := s.cfg.chain.ReceiveBlock(ctx, b, blkRoot); err != nil { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
...
func (s *Service) sendBatchRootRequest(ctx context.Context, roots [][32]byte, randGen *rand.Rand) error {
	...
	_, bestPeers := s.cfg.p2p.Peers().BestFinalized(maxPeerRequest, s.cfg.chain.FinalizedCheckpt().Epoch) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	if len(bestPeers) == 0 {
		return nil
	}
