...
func (s *Service) forkWatcher() {
	slotTicker := slots.NewSlotTicker(s.cfg.chain.GenesisTime(), params.BeaconConfig().SecondsPerSlot) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	for {
		select {
...
func (s *Service) registerForUpcomingFork(currEpoch types.Epoch) error {
	...
	isNextForkEpoch, err := forks.IsForkNextEpoch(s.cfg.chain.GenesisTime(), genRoot[:]) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	if err != nil {
