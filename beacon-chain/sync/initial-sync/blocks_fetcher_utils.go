...
import (
	...
	p2ppb "gitlab.waterfall.network/waterfall/protocol/coordinator/proto/prysm/v1alpha1"
...
func (f *blocksFetcher) findFork(ctx context.Context, slot types.Slot) (*forkData, error) {
	...
	finalizedEpoch := f.chain.FinalizedCheckpt().Epoch //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
...
func (f *blocksFetcher) findForkWithPeer(ctx context.Context, pid peer.ID, slot types.Slot) (*forkData, error) {
	...
	for i, block := range blocks {
		...
		if !f.db.HasBlock(ctx, parentRoot) && !f.chain.HasInitSyncBlock(parentRoot) { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
			log.WithFields(logrus.Fields{
				"peer": pid,
...
func (f *blocksFetcher) bestFinalizedSlot() types.Slot {
	finalizedEpoch, _ := f.p2p.Peers().BestFinalized(
		params.BeaconConfig().MaxPeersToSync, f.chain.FinalizedCheckpt().Epoch) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
