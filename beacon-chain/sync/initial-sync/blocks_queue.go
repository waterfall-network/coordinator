...
func (q *blocksQueue) loop() {
	...
	startSlot := q.chain.HeadSlot() //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	...
	for {
		...
		if q.chain.HeadSlot() >= q.highestExpectedSlot { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
		...
		log.WithFields(logrus.Fields{
			...
			"headSlot":            q.chain.HeadSlot(), //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
			"state":               q.smm.String(),
		...
		case <-ticker.C:
			for _, key := range q.smm.keys {
				...
				if q.chain.HeadSlot() >= fsm.start.Add(blocksPerRequest-1) { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
...
func (q *blocksQueue) onProcessSkippedEvent(ctx context.Context) eventHandlerFn {
	return func(m *stateMachine, in interface{}) (stateID, error) {
		...
		if q.mode == modeStopOnFinalizedEpoch {
			if bestFinalizedSlot <= q.chain.HeadSlot() { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
				return stateSkipped, errNoRequiredPeers
			}
		} else {
			if q.blocksFetcher.bestNonFinalizedSlot() <= q.chain.HeadSlot() { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
				return stateSkipped, errNoRequiredPeers
			}
		}

		// All machines are skipped, FSMs need reset.
		startSlot := q.chain.HeadSlot() + 1 //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
