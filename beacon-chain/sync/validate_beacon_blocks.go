...
func (s *Service) validateBeaconBlockPubSub(ctx context.Context, pid peer.ID, msg *pubsub.Message) (pubsub.ValidationResult, error) {
	...
	genesisTime := uint64(s.cfg.chain.GenesisTime().Unix()) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	...
	startSlot, err := slots.EpochStart(s.cfg.chain.FinalizedCheckpt().Epoch) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	if err != nil {
		log.WithError(err).WithField("blockSlot", blk.Block().Slot()).Debug("Ignored block")
		return pubsub.ValidationIgnore, nil
	}
	...
	if isBlockQueueable(genesisTime, blk.Block().Slot(), receivedTime) {
		...
		s.pendingQueueLock.Unlock()
		e := fmt.Errorf("early block, with current slot %d < block slot %d", s.cfg.chain.CurrentSlot(), blk.Block().Slot()) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
		return pubsub.ValidationIgnore, e
	}

	// Handle block when the parent is unknown.
	if !s.cfg.beaconDB.HasBlock(ctx, bytesutil.ToBytes32(blk.Block().ParentRoot())) && !s.cfg.chain.HasInitSyncBlock(bytesutil.ToBytes32(blk.Block().ParentRoot())) { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
		s.pendingQueueLock.Lock()
		if err := s.insertBlockToPendingQueue(blk.Block().Slot(), blk, blockRoot); err != nil {
			s.pendingQueueLock.Unlock()
			return pubsub.ValidationIgnore, err
...
func (s *Service) validateBeaconBlock(ctx context.Context, blk block.SignedBeaconBlock, blockRoot [32]byte) error {
	...
	if err := s.cfg.chain.VerifyBlkDescendant(ctx, bytesutil.ToBytes32(blk.Block().ParentRoot())); err != nil { //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
		s.setBadBlock(ctx, blockRoot)
		return err
	}
