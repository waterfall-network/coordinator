...
import (
	...
	testingDB "gitlab.waterfall.network/waterfall/protocol/coordinator/beacon-chain/db/testing"
...
func TestStatusRPCRequest_FinalizedBlockSkippedSlots(t *testing.T) {
	...
	require.NoError(t, err)
	bState, err := transition.GenesisBeaconState(context.Background(), nil, 0, &ethpb.Eth1Data{DepositRoot: make([]byte, 32), BlockHash: make([]byte, 32), Candidates: make([]byte, 0)})
	require.NoError(t, err)
...
func TestStatusRPCRequest_BadPeerHandshake(t *testing.T) {
	...
	assert.Equal(t, false, p1.Peers().Scorers().IsBadPeer(p2.PeerID()), "Peer is not marked as bad")
}
