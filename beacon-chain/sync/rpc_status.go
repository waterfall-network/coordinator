			...
			go func(id peer.ID) {
				...
				if prysmTime.Now().After(lastUpdated.Add(interval)) {
					if err := s.reValidatePeer(s.ctx, id); err != nil {
						...
						log.WithField("fn", "maintainPeerStatuses").WithField("peer", id.String()).WithError(err).Info("Disconnect: incr BadResponses")
...
func (s *Service) sendRPCStatusRequest(ctx context.Context, id peer.ID) error {
	...
	if code != 0 {
		...
		log.WithField("fn", "sendRPCStatusRequest").WithField("peer", id.String()).WithError(err).Info("Disconnect: incr BadResponses")
...
func (s *Service) statusRPCHandler(ctx context.Context, msg interface{}, stream libp2pcore.Stream) error {
	...
	if err := s.validateStatusMessage(ctx, m); err != nil {
		...
		default:
			...
			log.WithField("fn", "statusRPCHandler").WithField("peer", remotePeer.String()).WithError(err).Info("Disconnect: incr BadResponses")
		}
