...
func (s *Service) processPendingAtts(ctx context.Context) error {
	...
	s.validatePendingAtts(ctx, s.cfg.chain.CurrentSlot()) // nolint
		...
		if s.cfg.beaconDB.HasBlock(ctx, bRoot) && (s.cfg.beaconDB.HasState(ctx, bRoot) || s.cfg.beaconDB.HasStateSummary(ctx, bRoot)) {
			for _, signedAtt := range attestations {
				...
				if helpers.IsAggregated(att.Aggregate) {
					...
					if err := s.cfg.chain.VerifyFinalizedConsistency(ctx, att.Aggregate.Data.BeaconBlockRoot); err != nil { // nolint
						log.WithError(err).Debug("Could not verify finalized consistency")
						continue
					}
					if err := s.cfg.chain.VerifyLmdFfgConsistency(ctx, att.Aggregate); err != nil { // nolint
						log.WithError(err).Debug("Could not verify FFG consistency")
						continue
					}
					preState, err := s.cfg.chain.AttestationTargetState(ctx, att.Aggregate.Data.Target) // nolint
					if err != nil {
			...
			log.WithFields(logrus.Fields{
				"currentSlot": s.cfg.chain.CurrentSlot(), // nolint
