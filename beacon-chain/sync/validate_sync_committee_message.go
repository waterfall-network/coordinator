...
func (s *Service) validateSyncCommitteeMessage(
	...
	if err := altair.ValidateSyncMessageTime(
		m.Slot,
		s.cfg.chain.GenesisTime(), //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	...
	committeeIndices, err := s.cfg.chain.HeadSyncCommitteeIndices(ctx, m.ValidatorIndex, m.Slot) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
	if err != nil {
		tracing.AnnotateError(span, err)
		return pubsub.ValidationIgnore, err
	}
...
func (s *Service) rejectInvalidSyncCommitteeSignature(m *ethpb.SyncCommitteeMessage) validationFn {
	return func(ctx context.Context) (pubsub.ValidationResult, error) {
		...
		d, err := s.cfg.chain.HeadSyncCommitteeDomain(ctx, m.Slot) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
		if err != nil {
			tracing.AnnotateError(span, err)
			return pubsub.ValidationIgnore, err
		}
		...
		pubKey, err := s.cfg.chain.HeadValidatorIndexToPublicKey(ctx, m.ValidatorIndex) //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
		if err != nil {
			tracing.AnnotateError(span, err)
			return pubsub.ValidationReject, err
		}
