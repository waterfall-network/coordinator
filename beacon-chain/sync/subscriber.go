...
func (s *Service) subscribe(topic string, validator wrappedVal, handle subHandler, digest [4]byte) *pubsub.Subscription {
	genRoot := s.cfg.chain.GenesisValidatorsRoot() //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
			...
			case <-ticker.C():
				...
				if err != nil {
					log.WithError(err).WithFields(logrus.Fields{
						...
						"isDigestValid": valid,
						"topic":         topic,
						"s.curSlot":     s.cfg.chain.CurrentSlot(),                                   //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
						"curSlot":       slots.CurrentSlot(uint64(s.cfg.chain.GenesisTime().Unix())), //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
...
func (s *Service) subscribeDynamicWithSubnets(
	...
	log.WithFields(logrus.Fields{
		...
		"3:s.curSlot":   s.cfg.chain.CurrentSlot(),                                   //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
		"4:calcSlot":    slots.CurrentSlot(uint64(s.cfg.chain.GenesisTime().Unix())), //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
...
func (s *Service) lookupAttesterSubnets(digest [4]byte, idx uint64) {
	...
	if !s.validPeersExist(subnetTopic) {
		log.WithFields(logrus.Fields{
			...
			"digest":      fmt.Sprintf("%#x", digest),
			"curSlot":     s.cfg.chain.CurrentSlot(), //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
...
func (s *Service) filterNeededPeers(pids []peer.ID) []peer.ID {
	...
	currSlot := s.cfg.chain.CurrentSlot() //nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
...
// nolint: typecheck // Linter does not determine nesting of interfaces (interface blockchainService)
