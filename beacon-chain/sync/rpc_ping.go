...
func (s *Service) pingHandler(_ context.Context, msg interface{}, stream libp2pcore.Stream) error {
	...
	if err != nil {
		...
		if errors.Is(err, p2ptypes.ErrInvalidSequenceNum) {
			...
			log.WithField("fn", "pingHandler").WithField("peer", stream.Conn().RemotePeer().String()).WithError(err).Info("Disconnect: incr BadResponses")
...
func (s *Service) sendPingRequest(ctx context.Context, id peer.ID) error {
	...
	if code != 0 {
		log.WithField("fn", "sendPingRequest:ReadStatusCode").WithField("peer", id.String()).WithField("code", code).WithError(err).Info("Disconnect: incr BadResponses")
		s.cfg.p2p.Peers().Scorers().BadResponsesScorer().Increment(stream.Conn().RemotePeer())
	...
	if err != nil {
		// Descore peer for giving us a bad sequence number.
		if errors.Is(err, p2ptypes.ErrInvalidSequenceNum) {
			log.WithField("fn", "sendPingRequest:validateSequenceNum").WithField("peer", id.String()).WithError(err).Info("Disconnect: incr BadResponses")
